...
Column이 아닌 ListView로 전체화면을 구성하는 이유 중 하나로는 화면에 TextFormField가 있는 경우 TextFormField를 터치할 경우 아래에서 키보드가 올라오게 되는데, 키보드가 올라오면서 inset이라는 화면에 그림을 그릴 수 없는 영역이 생기게 되어 화면을 스크롤 할 수 있도록 ListView로 만들어주면 더 좋다.
...
Stack 방식을 이용해 위젯을 쌓아 여러 위젯을 관리하는 위젯이다.
